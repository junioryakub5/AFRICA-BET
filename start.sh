#!/bin/bash
# ─── Africa Tips — One-command local startup ─────────────────────────────────────
...
echo ""
echo "🟢 Africa Tips — Starting up..."
echo "─────────────────────────────────────"

# ── 1. Kill anything already running on Africa Tips ports ──────────────────────
echo "⏹  Stopping old processes..."
lsof -ti:5004 | xargs kill -9 2>/dev/null || true
lsof -ti:3003 | xargs kill -9 2>/dev/null || true
ps aux | grep 'cloudflared.*5004\|cloudflared.*3003' | grep -v grep | awk '{print $2}' | xargs kill -9 2>/dev/null || true
sleep 1

# ── 2. Start backend ──────────────────────────────────────────────────────────
echo "⚙️  Starting backend on port 5004..."
cd "$BACKEND_DIR"
node server.js > /tmp/africa-tips-backend.log 2>&1 &
BACKEND_PID=$!
...
cloudflared tunnel --url http://localhost:5004 > /tmp/africa-tips-tunnel-backend.log 2>&1 &
sleep 8

BACKEND_URL=$(grep -o 'https://[a-z0-9-]*\.trycloudflare\.com' /tmp/africa-tips-tunnel-backend.log | head -1)
if [ -z "$BACKEND_URL" ]; then
  echo "❌ Could not get backend tunnel URL. Check /tmp/africa-tips-tunnel-backend.log"
  exit 1
fi
...
cd "$FRONTEND_DIR"
PORT=3003 npm run dev > /tmp/africa-tips-frontend.log 2>&1 &
FRONTEND_PID=$!
...
cloudflared tunnel --url http://localhost:3003 > /tmp/africa-tips-tunnel-frontend.log 2>&1 &
sleep 8

FRONTEND_URL=$(grep -o 'https://[a-z0-9-]*\.trycloudflare\.com' /tmp/africa-tips-tunnel-frontend.log | head -1)
if [ -z "$FRONTEND_URL" ]; then
  echo "❌ Could not get frontend tunnel URL. Check /tmp/africa-tips-tunnel-frontend.log"
  exit 1
fi
